...
## Overview
The robot navigates from a user-selected corner to the center of the pool table, which is used as the reference point for detecting and aligning with a pool ball. It precisely positions itself, orbits to the optimal angle relative to the nearest pocket, and strikes the ball into the pocket using a striker mechanism. This process repeats with periodic recalibration until all balls are cleared.
